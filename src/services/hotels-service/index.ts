...
async function listHotels(userId: number) {
  ...
  if (ticket.TicketType.isRemote) {
    throw requestError(400, "OnlineTicket");
  }
...
async function getHotels(userId: number) {
  ...
  for (let j = 0; j < hotels.length; j++) {
    ...
    for (let i = 0; i < hotels[j].Rooms.length; i++) {
      if (hotels[j].Rooms[i].capacity === 1 && !typeArray.includes("Single")) typeArray.push("Single");
      if (hotels[j].Rooms[i].capacity === 2 && !typeArray.includes("Double")) typeArray.push("Double");
      if (hotels[j].Rooms[i].capacity === 3 && !typeArray.includes("Triple")) typeArray.push("Triple");

      hotelType.vacancy += await roomVacancy(hotels[j].Rooms[i].id);
    }

    if (typeArray.length === 1) hotelType.type = typeArray[0];
    if (typeArray.length === 2)
      if (typeArray[0] === "Single" || typeArray[1] === "Triple") hotelType.type = typeArray[0] + " e " + typeArray[1];
      else hotelType.type = typeArray[1] + " e " + typeArray[0];
    if (typeArray.length === 3) hotelType.type = "Single, Double e Triple";
